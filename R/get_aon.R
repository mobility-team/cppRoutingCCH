...
#'
#' @param Graph  An object generated by \link{makegraph}, \link{cpp_contract} or \link{cpp_cch_customize} function.
...
#' @param algorithm character. For contracted network : \code{phast} or \code{bi}. Ignored for a CCH metric, which always uses the CCH elimination-tree query. Otherwise : \code{d}, \code{bi} or \code{nba}. Default to \code{bi}. See details.
...
#'
#' On a CCH metric generated by \link{cpp_cch_customize}, this function uses
#' the elimination-tree CCH query. This is the query variant used by
#' \link{assign_traffic} with \code{aon_method = "cch"}.
#'
...
get_aon<-function(Graph, from, to, demand, algorithm = "bi",constant = 1){
  ...
  if (inherits(Graph, "cppRouting_cch_metric")){
    res <- cppaoncchelim(Graph$original$data$from,
